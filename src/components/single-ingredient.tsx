  ...
  return (
    <div className="space-y-4">
      <div className="text-center pb-2">
        <h3 className="text-white font-semibold">{tx.si_title}</h3>
        <p className="text-gray-500 text-xs mt-1">{tx.si_subtitle}</p>
      </div>
      ...
      {aiLoading && (
        <div className="bg-purple-950/30 border border-purple-500/30 rounded-xl p-4 text-center">
          <div className="text-purple-400 text-sm animate-pulse">{tx.si_ai_analyzing.replace("{q}", query)}</div>
      ...
      {!result && !aiResult && !aiLoading && (
        <div>
          <p className="text-xs text-gray-600 mb-2">{tx.si_popular}</p>
      ...
      {p && !aiLoading && (
        <div className="space-y-4">
          <div className="flex items-start justify-between">
            ...
            {p.isAI ? (
              <div className="flex items-center gap-1 bg-purple-950/50 border border-purple-500/30 rounded-lg px-2 py-1">
                <span className="text-purple-400 text-xs">{tx.dr_ai_estimate}</span>
              </div>
            ) : (
              <div className={`text-xs flex items-center gap-1 ${p.confidence >= 0.9 ? "text-green-400" : p.confidence >= 0.75 ? "text-teal-400" : "text-amber-400"}`}>
                ...
                <span>{p.confidence >= 0.9 ? tx.si_lab_verified : tx.ua_conf_high}</span>
              </div>
            )}
          </div>
          ...
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800 text-center">
            <div className="text-xs text-gray-500 mb-1">{tx.si_gl_for} {portionG}g</div>
            <div className={`text-5xl font-bold ${riskColor}`}>{p.gl}</div>
            <div className={`text-sm mt-1 ${riskColor}`}>
              {p.gl < 10 ? tx.si_impact_low : p.gl <= 20 ? tx.si_impact_med : tx.si_impact_high} {tx.si_glycemic_impact}
            </div>
            <div className="text-xs text-gray-600 mt-2">GI: {p.gi} · {tx.si_net_carbs} {p.netCarb}g</div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: tx.si_carbs, value: `${p.carbs}g`, color: "text-blue-400" },
              { label: tx.fiber, value: `${p.fiber}g`, color: "text-green-400" },
              { label: tx.ua_net_carb, value: `${p.netCarb}g`, color: "text-teal-400" },
              { label: tx.ua_protein, value: `${p.protein}g`, color: "text-purple-400" },
              { label: tx.ua_fat, value: `${p.fat}g`, color: "text-amber-400" },
              { label: tx.at_calories, value: `${p.cal}kcal`, color: "text-orange-400" },
            ].map((s) => (
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSave} disabled={saved}
              ...
              {saved ? tx.si_saved : tx.dr_log_history}
            </button>
          </div>
        </div>
      )}
