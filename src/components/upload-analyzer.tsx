...
import { t, detectBrowserLang, type Lang } from "@/lib/i18n";
...
export function UploadAnalyzer({ userType = "healthy" }: { userType?: string }) {
  ...
  const handleFile = useCallback((file: File) => {
    if (!file.type.startsWith("image/")) { setError(t[lang].error_image); return; }
    if (file.size > 5 * 1024 * 1024) { setError(t[lang].error_size); return; }
  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t[lang].error_failed);
      setResult(data.analysis);
    } catch (e) {
      setError(e instanceof Error ? e.message : t[lang].error_failed);
    } finally {
      setLoading(false);
    }
  };
